...
async def setup_hook():
    ...
    try:
        ...
        # ===== Persistent views with message_id =====
    views_map = {
        'join': SetupJoinView(),
        'report': ReportTicketView(),
        'anon': AnonTicketView(),
        'qna': QnaTicketView(),
        'coupon': CouponPanelView(),
        'util': UtilView(),
    }
    # load saved panel IDs
    panel_data = {}
    if os.path.exists(PANEL_FILE):
        try:
            with open(PANEL_FILE, 'r', encoding='utf-8') as f:
    ...
    for name, view in views_map.items():
        if name in panel_data:
            try:
                bot.add_view(view, message_id=int(panel_data[name][1]))
                continue
            except:
                pass
        bot.add_view(view)
    
    # other existing views (keep original)
